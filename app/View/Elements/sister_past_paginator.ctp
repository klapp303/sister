...
//ページ数が少ない場合
    ...
    for ($i = 1; $i < $paginator_setting['current_page']; $i++) {
        echo $this->Html->link($i, '/diary/past/page:' . $i);
...
//始めの場合
    ...
    echo $this->Html->link($paginator_option['last'], '/diary/past/page:' . $paginator_setting['max_page']);
    
//終わりの場合
} elseif ($paginator_setting['current_page'] >= $paginator_setting['max_page'] - floor($paginator_option['modulus'] /2)) {
    echo $this->Html->link($paginator_option['first'], '/diary/past/page:1');
