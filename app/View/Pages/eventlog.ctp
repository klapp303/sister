...
<p class="intro_evelog">
  ...
  特に推してるのは竹達彩奈さん、内田真礼さん、麻倉ももさん、伊藤美来さん、稗田寧々さん。<br>
  <?php if (!@$cr_year): ?>
...
$array_data_cat_2 = array(
    '竹達彩奈さん' => 'ayachi', '内田真礼さん' => 'taso', '麻倉ももさん' => 'mocho', '伊藤美来さん' => 'mikku', '稗田寧々さん' => 'nenemaru'
);
?>
<h4>参加履歴データ</h4>
    ...
    foreach ($eventlog['schedule'][$year] as $month => $val) {
        foreach ($val as $event) {
            ...
            foreach ($event['cast'] as $cast) {
                ...
                if ($cast == '稗田寧々' || $cast == 'DIALOGUE+' || $cast == 'AiRBLUE') {
                    $data['nenemaru']++;
                }
            }
        }
